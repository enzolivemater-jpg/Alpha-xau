/**
 * =============================================================================
 *  ALPHA-XAU — backend/event_impact/shadow_runtime.ts
 *
 *  EI-6 — Controlled Manual Worker Runtime (V1) for Event Impact.
 *
 *  Wires the merged EI-5 controlled explicit batch runner
 *  (backend/event_impact/shadow_batch.ts, imported and reused here, never
 *  reimplemented) into a CONTROLLED MANUAL authenticated HTTP endpoint:
 *
 *    POST /event-impact-shadow
 *      -> authenticate operator (reuses the EXISTING INGEST_TOKEN /news
 *         and /event-shadow manual-trigger convention — never a new
 ...
 *         this module — mirrors backend/event_engine/shadow_runtime.ts's
 *         adapter EXACTLY; no exported reusable client exists in the repo
 *         to reuse without introducing coupling)
 *      -> runEventImpactShadowBatch(db, eventVersionIds, 'manual') — the
 *         trigger is ALWAYS the literal 'manual', NEVER read from the
 *         request body
 *      -> map the result to a safe structured HTTP response
 *
 *  RESPONSIBILITIES ONLY: HTTP authentication, method/envelope validation,
 *  a minimal real PostgREST DB adapter, invoking EI-5, and safe response
 *  mapping. This module NEVER duplicates EI-2/EI-3/EI-4/EI-5 logic and
 *  NEVER calls the Event Impact persistence RPC
 *  (fn_event_impact_create_assessment) directly — every RPC call stays
 *  inside EI-2/EI-4. It never decides direction/magnitude/confidence/
 *  horizon/pricing_state, never maps news_score/expected_move_usd/
 *  gold_direction_impact/Committee probability/target/market_regime,
 *  never infers Gold Transmission, never calls an LLM.
 *
 ...
 *  Event Impact candidate discovery/backlog selector of any kind — EI-5
 *  accepts explicit Event Version IDs only, and this runtime forwards
 ...
 *  `path === '/event-impact-shadow'` — deliberately NOT
 *  `startsWith('/event-impact-shadow')`. No sub-routes, no automatic
 *  selector, unlike OPS-023's separate PR9 discover endpoint (which has
 *  no Event Impact equivalent here).
 *
 *  Current deterministic V1 behavior remains intentionally conservative
 *  (EI-3): a valid Event Version resolves to INSUFFICIENT_EVIDENCE with
 *  zero interpretations, unless EI-3 emits UNAVAILABLE for a supported
 *  reason. This runtime does not, and must not, widen that behavior.
 * =============================================================================
 */
...
export interface EventImpactShadowRuntimeEnv {
...
/** The trigger EI-5 ALWAYS receives from this runtime. Never read from the
 *  request body — see parseEnvelope() below, which accepts no such field. */
const EVENT_IMPACT_SHADOW_RUNTIME_TRIGGER_TYPE = 'manual' as const;
...
// backend/event_engine/shadow_runtime.ts EXACTLY). Never lets a
// service-role key, an INGEST_TOKEN, or an Authorization/apikey header
// value reach a log line or an HTTP response.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Authentication — mirrors the EXISTING /news and /event-shadow manual-
// trigger convention EXACTLY: same header contract (x-ingest-token, or
// Authorization: Bearer <token> fallback), same constant-time comparison,
// same token (INGEST_TOKEN). No new token, no new auth scheme in EI-6.
// ---------------------------------------------------------------------------
...
function isAuthorized(request: Request, env: EventImpactShadowRuntimeEnv): boolean {
...
// ---------------------------------------------------------------------------
// Request envelope — SHAPE ONLY. EI-5 remains authoritative for semantic
// validation (empty list, >25 ids, UUID validity, duplicate ids) — never
// duplicated here.
// ---------------------------------------------------------------------------

export interface EventImpactShadowRequestEnvelope {
  readonly eventVersionIds: readonly unknown[];
}

class EventImpactShadowRuntimeEnvelopeError extends Error {
  readonly code: string;
  constructor(code: string, message: string) {
    super(message);
    this.name = 'EventImpactShadowRuntimeEnvelopeError';
    this.code = code;
  }
}
...
/**
 * Transport-level bound, deliberately NOT a duplication of EI-5's semantic
 * limits (max 25 ids / UUID validity / duplicates — all still EI-5's
 ...
 * same bound as backend/event_engine/shadow_runtime.ts.
 */
export const MAX_EVENT_IMPACT_SHADOW_REQUEST_BODY_BYTES = 16 * 1024;
...
function checkDeclaredContentLength(request: Request): void {
  ...
  if (Number.isFinite(declared) && declared > MAX_EVENT_IMPACT_SHADOW_REQUEST_BODY_BYTES) {
    throw new EventImpactShadowRuntimeEnvelopeError(
      'REQUEST_BODY_TOO_LARGE',
      `Request body exceeds the ${MAX_EVENT_IMPACT_SHADOW_REQUEST_BODY_BYTES}-byte limit (Content-Length: ${header}).`,
    );
  }
}
...
async function readBoundedRequestBody(request: Request): Promise<string> {
  ...
  if (byteLength > MAX_EVENT_IMPACT_SHADOW_REQUEST_BODY_BYTES) {
    throw new EventImpactShadowRuntimeEnvelopeError(
      'REQUEST_BODY_TOO_LARGE',
      `Request body exceeds the ${MAX_EVENT_IMPACT_SHADOW_REQUEST_BODY_BYTES}-byte limit.`,
    );
  }
  return text;
}

async function parseEnvelope(request: Request): Promise<EventImpactShadowRequestEnvelope> {
  ...
  try {
    ...
    throw new EventImpactShadowRuntimeEnvelopeError('MALFORMED_JSON', 'Request body is not valid JSON.');
  }

  if (typeof raw !== 'object' || raw === null || Array.isArray(raw)) {
    throw new EventImpactShadowRuntimeEnvelopeError('INVALID_BODY_SHAPE', 'Request body must be a JSON object.');
  }
  const obj = raw as Record<string, unknown>;

  if (!('eventVersionIds' in obj)) {
    throw new EventImpactShadowRuntimeEnvelopeError('MISSING_EVENT_VERSION_IDS', 'Request body must include "eventVersionIds".');
  }
  ...
  if (!Array.isArray(obj.eventVersionIds)) {
    throw new EventImpactShadowRuntimeEnvelopeError('EVENT_VERSION_IDS_NOT_ARRAY', '"eventVersionIds" must be an array.');
  }
...
// is a fresh minimal implementation of the SAME EventImpactShadowBatchDb/
// LockCapableDb structural `request<T>(method, path, body?, extraHeaders?)`
// contract EI-2/EI-4/EI-5/run_lock.ts already depend on — deliberately NOT
// a refactor of backend/event_engine/shadow_runtime.ts's adapter to share
// code, which would widen this PR unnecessarily; the logic is intentionally
// duplicated, mirroring that adapter's already-reviewed shape exactly.
// ---------------------------------------------------------------------------

export class EventImpactShadowDbError extends Error {
  readonly status?: number;
  constructor(message: string, status?: number) {
    super(message);
    this.name = 'EventImpactShadowDbError';
    this.status = status;
  }
}
...
export function validateSupabaseUrl(rawUrl: string): URL {
  ...
  try {
    ...
    throw new EventImpactShadowDbError('SUPABASE_URL is not a valid URL.');
  }
  if (parsed.protocol !== 'https:') {
    throw new EventImpactShadowDbError('SUPABASE_URL must use the https: protocol.');
  }
  return parsed;
}
...
export function validatePostgrestPath(path: string): void {
  if (path.includes('://') || path.includes('..')) {
    throw new EventImpactShadowDbError('PostgREST path rejected: potential escape sequence.');
  }
}
 ...
 *  call — matching backend/event_engine/shadow_runtime.ts's PR7 auth
 *  hotfix exactly. */
...
export class PostgrestEventImpactShadowDb implements EventImpactShadowBatchDb {
  ...
  async request<T>(
    ...
    if (!SUPPORTED_DB_METHODS.has(method)) {
      throw new EventImpactShadowDbError(`Unsupported PostgREST method: ${String(method)}. Only GET, POST, PATCH are allowed.`);
    }
    ...
    if (!response.ok) {
      ...
      // — not even redacted. This error can be caught per-item by EI-5 and
    ...
    try {
      ...
      throw new EventImpactShadowDbError('PostgREST returned malformed JSON for a successful (2xx) response.');
    }
  }
}
...
/** EI-5 invariant codes that originate from CALLER input (the request
 *  envelope's eventVersionIds) — mapped to HTTP 400. Every other
 *  EventImpactShadowBatchInvariantError (e.g.
 *  EVENT_IMPACT_SHADOW_LOCK_RELEASE_FAILED) is an operational failure,
 *  mapped to HTTP 500. */
...
export async function handleEventImpactShadowRequest(
  request: Request,
  env: EventImpactShadowRuntimeEnv,
): Promise<Response> {
  ...
  let envelope: EventImpactShadowRequestEnvelope;
  try {
    envelope = await parseEnvelope(request);
  } catch (err) {
    if (err instanceof EventImpactShadowRuntimeEnvelopeError) {
  ...
  let db: EventImpactShadowBatchDb;
  try {
    db = new PostgrestEventImpactShadowDb(env);
  } catch {
    return jsonResponse({ error: 'Event-impact-shadow runtime configuration error.' }, 500);
  }

  try {
    // The trigger is ALWAYS the literal 'manual' — never from the request
    // body, never 'cron'/'webhook'/'backfill' via this runtime path.
    const report = await runEventImpactShadowBatch(
      db,
      envelope.eventVersionIds as readonly string[],
      EVENT_IMPACT_SHADOW_RUNTIME_TRIGGER_TYPE,
    );
    ...
    // same established manual Event Shadow operational-report convention.
    return jsonResponse({ ok: true, report }, 200);
  } catch (err) {
    if (err instanceof EventImpactShadowBatchBusyError) {
      return jsonResponse({ error: err.message, code: 'EVENT_IMPACT_SHADOW_ALREADY_RUNNING' }, 409);
    }
    if (err instanceof EventImpactShadowBatchInvariantError) {
      if (CALLER_INPUT_INVARIANT_CODES.has(err.code)) {
        return jsonResponse({ error: err.message, code: err.code }, 400);
      }
      return jsonResponse({ error: 'Event-impact-shadow batch failed.', code: err.code }, 500);
    }
    ...
    return jsonResponse({ error: 'Internal event-impact-shadow runtime error.' }, 500);
  }
}
