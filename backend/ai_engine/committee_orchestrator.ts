...
async function callClaude(
  ...
  for (let attempt = 0; attempt <= CONFIG.MAX_RETRIES; attempt++) {
    ...
    try {
      const response = await fetch(CONFIG.ANTHROPIC_URL, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            // Prefill : force la sortie à démarrer par un objet JSON.
      ...
      try {
        // Réinjection du prefill consommé par le tour assistant.
...
export async function runCommittee(
  ...
  try {
    return await runCommitteeLocked(env, db, log, scope, startedAt, agents);
  } finally {
...
export async function handleCommitteeEvent(raw: unknown, env: Env): Promise<EventResult> {
  ...
  try {
    ...
    // Libération inconditionnelle du verrou.
    await releaseLock(db, lock.runRowId, {
