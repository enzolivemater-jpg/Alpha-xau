 ...
 *  DXY         RETIRÉ — voir bloc dédié ci-dessous
 *
 ...
 *  Twelve Data.
 *
 *  RETRAIT DE DXY (2026-08-18) — symbole absent chez Twelve Data, vérifié
 *  par leur propre API de recherche, pas supposé.
 *  `GET /quote?symbol=DXY` renvoyait HTTP 404 « symbol parameter is
 *  missing or invalid » à chaque cycle. `GET /symbol_search?symbol=dollar
 *  index` ne remonte que des ETF proxy (UUP, UDN — Invesco DB US Dollar
 *  Index Bullish/Bearish Fund), jamais l'indice ICE lui-même : Twelve Data
 *  ne le catalogue tout simplement pas comme instrument interrogeable.
 *  Un ETF a été envisagé comme proxy et écarté : coté uniquement pendant
 *  les heures NYSE, il resterait figé (avec un horodatage « frais »
 *  trompeur) en dehors — soit précisément le défaut qu'on vient de
 *  corriger sur XAUUSD via `last_quote_at`, réintroduit sous une autre
 *  forme. La collecte est donc purement et simplement retirée :
 *  `dxy_value` reste NULL en base, affiché N/A par le frontend. Le driver
 *  Real Yield (DFII10) reste le driver macro dominant documenté pour l'or
 *  et capture indirectement une partie du signal dollar (taux réels =
 *  nominal + anticipations d'inflation).
 *
 *  ATTENTION QUOTA — la clé Twelve Data gratuite est plafonnée à 800
 *  requêtes/jour. Le retrait de DXY ramène XAUUSD seul à ~144 req/jour au
 *  rythme du cron toutes les 5 minutes, largement sous le plafond ;
 *  aucun arbitrage de cadence n'est plus nécessaire pour cette raison.
 *
...
/* -------------------------------------------------------------------------- */
/*  Provider 3 — Twelve Data (XAUUSD)                                          */
/*  DXY retiré du champ d'action de ce provider — voir le bloc de décision     */
/*  en tête de fichier (symbole absent chez Twelve Data, vérifié).             */
/* -------------------------------------------------------------------------- */
...
export async function fetchTwelveData(
  ...
  if (!env.TWELVE_DATA_KEY) {
    return {
      ...
      value: null,
      observedAt: null,
      unavailableReason: 'TWELVE_DATA_KEY absente : XAUUSD indisponible.',
    };
  }
