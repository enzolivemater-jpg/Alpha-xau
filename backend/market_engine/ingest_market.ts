...
const CONFIG = {
  ...
  DB_MAX_RETRIES: 2,
  /** Symbole Twelve Data de l'or spot. Stooq retiré (défi anti-bot). */
  TWELVE_XAUUSD: 'XAU/USD',
 ...
 * /symbol_search — vérifié, pas supposé). La colonne `dxy_value` reste
 * dans le schéma, en lecture NULL permanente ; le frontend l'affiche déjà
 * honnêtement en N/A plutôt que d'inventer une valeur.
 */
...
export async function collectAll(
  ...
  const tasks: Array<{ name: string; run: () => Promise<RawDataPoint> }> = [
    { name: 'twelve_data:XAUUSD', run: () => fetchTwelveData('XAUUSD', CONFIG.TWELVE_XAUUSD, env) },
    ...FRED_SERIES.map((s) => ({
      name: `fred:${s.symbol}`,
