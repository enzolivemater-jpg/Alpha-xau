/**
 * =============================================================================
 *  ALPHA-XAU — backend/news_sources/us_treasury_raw.ts
 *
 *  Adaptateur d'orchestration RAW : U.S. Treasury Press Releases collector
 *  -> writeRawNewsObservations() -> public.news_articles.
 *
 *  Petit adaptateur volontairement mince : AUCUNE logique métier propre.
 *  Il ne fait qu'appeler collectTreasuryNews() puis
 *  writeRawNewsObservations(), et agrège un résumé déterministe. Même
 *  contrat/forme que backend/news_sources/federal_reserve_raw.ts et
 *  backend/news_sources/ecb_raw.ts (délibérément dupliqué, pas
 *  factorisé — Treasury et OFAC gardent des contrats de provenance
 *  distincts, pas d'abstraction générique introduite ici).
 *
 ...
 *  Cluster). Les observations Treasury n'entrent JAMAIS dans le pipeline
...
export interface TreasuryRawIngestOptions {
...
export interface TreasuryRawIngestResult {
  readonly ok: boolean;
...
/**
 * ok=true UNIQUEMENT si : la page Treasury est status='ok', ET aucun
 ...
 */
export async function ingestTreasuryRaw(
  options: TreasuryRawIngestOptions,
): Promise<TreasuryRawIngestResult> {
  const startedAt = Date.now();

  const collected = await collectTreasuryNews({
  ...
  // même règle que federal_reserve_raw.ts, aucune donnée d'article dans
  // les logs/erreurs.
